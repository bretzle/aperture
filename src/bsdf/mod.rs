...
pub use self::{bxdf::*, fourier::*, fresnel::*, lambertian::*, microfacet::*, oren_nayar::*};
...
impl<'a> BxDFHolder<'a> {
    pub fn new(arena: &'a Allocator<'_>) -> Self {
        Self {
...
impl<'a> BSDF<'a> {
    ...
    pub fn f(&self, wo_w: &Vector3f, wi_w: &Vector3f, flags: BxDFType) -> Spectrum {
        ...
        self.bxdfs
            ...
            .fold(Colors::BLACK, |c, b| c + b.f(&wo, &wi))
    }
    ...
    pub fn sample_f(
        ...
        if !bxdf.get_type().contains(BxDFType::BSDF_SPECULAR) {
            ...
            f = matching_comps
                ...
                .fold(Colors::BLACK, |f, b| f + b.f(&wo, &wi));
        }
