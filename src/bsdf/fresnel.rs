use crate::{
    bsdf::{BxDF, BxDFType, MicrofacetDistribution},
    geometry::*,
...
impl<'a> FresnelBlend<'a> {
    ...
    fn schlick_fresnel(&self, cos_theta: f32) -> Spectrum {
        self.rs + pow5(1.0 - cos_theta) * (Colors::WHITE - self.rs)
    }
}
...
mod tests {
    use super::*;
    use crate::{bsdf::TrowbridgeReitzDistribution, geometry::Vector3f, spectrum::Colors};
    ...
    quickcheck! {
        fn pdf_should_be_positive(x1: f32, y1: f32, z1: f32, x2: f32, y2: f32, z2: f32) -> bool {
            ...
            let fresnel = FresnelBlend::new(Colors::WHITE, Colors::WHITE, &distrib);
